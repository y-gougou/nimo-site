...
## 二、三大核心概念
    ...
    容器 Container = 运行实例（模具做出来的蛋糕）—— 可删除、可重建
...
| `whoami` | root | 容器内默认 root；通常受隔离约束，但不是绝对安全边界 |
...
!!! warning "隔离不是绝对安全"
    ...
    如果容器挂载了宿主敏感目录、Docker Socket，或使用 `--privileged`，容器内进程仍可能影响宿主。容器内 root 也不应被理解成“怎么操作都安全”。
