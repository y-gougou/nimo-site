# 2026-08-18 · DeepSeek Harness：从安装到故障排查的一天

> 📅 **学习日志** · 2026 年 8 月 18 日 · 自动化专业 / 工具链实践

!!! abstract "一句话总结"

    今天以 DeepSeek Harness（dsh）为载体，把 Linux 工具链、GitHub 用法、进程与端口、故障排查方法论、systemd 服务管理完整走了一遍。装一个框架，学了一串底层概念。
...
| 6 | 插件广场探索 | dsh-market 是插件市场；用 GraphQL 批量查 star 排行 |
| 7 | 故障排查方法论 | 症状 → 查配置 → 看插件树 → 定位根因（版本不匹配） |
| 8 | systemd 服务管理 | 一条 `systemctl --user restart` 告别手动 kill |
| 9 | Obsidian 笔记库 | vault = 普通文件夹；Markdown 与网站工作流同源 |
| 10 | API 配置排查 | 学校网关要配自定义端点；凭据解析有优先级 |

---

## 一、认识 DeepSeek Harness（agent harness 是什么）
...
- **验证要实测**：`curl 网关/models -H "Authorization: Bearer <key>"` 返回模型列表 = 全链路通

## 九、今日总结

今天没有学"一个新工具"，而是**用 dsh 把一堆底层概念串了起来**：包管理、构建、进程、端口、GitHub、插件生态、故障排查、服务管理、笔记体系、API 认证。最值钱的是那套排查方法论 —— 遇到"装上了但没用"的问题，先看证据（配置、插件树、日志），再下结论，别靠猜。
